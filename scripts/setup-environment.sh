...
echo "🔧 Director Environment Setup"
echo "============================"
...
DIRECTOR_DIR=$(basename $(pwd))
ENV_FILE=".env"

# Update Director's .env file with detected environment  
echo "📝 Updating Director's .env file..."
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    rm -f "$ENV_FILE.tmp"
    echo "✅ Updated WORKSPACE_ROOT in Director's .env"
else
...
DIRECTOR_HOME=\${WORKSPACE_ROOT}/$DIRECTOR_DIR
# Add actor paths as needed:
# ACTOR1_HOME=\${WORKSPACE_ROOT}/actor1
# ACTOR2_HOME=\${WORKSPACE_ROOT}/actor2

# MCP Configuration  
MCP_CLI_DIR=\${WORKSPACE_ROOT}/$DIRECTOR_DIR/apps/mcp/cli_use
...
EOF
    echo "✅ Created new Director .env file"
fi

# Test environment
echo "🧪 Testing environment..."
export WORKSPACE_ROOT="$WORKSPACE_ROOT"
# Source Director's .env for testing
source "$ENV_FILE"

# Check paths (flat git submodule structure)
if [[ -d "$WORKSPACE_ROOT/$DIRECTOR_DIR" ]]; then
    echo "✅ Director submodule found"
else
    echo "❌ Director submodule not found at $WORKSPACE_ROOT/$DIRECTOR_DIR"
fi

if [[ -d "$WORKSPACE_ROOT/$DIRECTOR_DIR/scripts" ]]; then
    echo "✅ Director scripts directory found"
else
    echo "❌ Director scripts directory not found at $WORKSPACE_ROOT/$DIRECTOR_DIR/scripts"
fi
...
echo ""
echo "Next steps:"
echo "1. Update .env with your tokens if needed"
echo "2. Run: source .env"
echo "3. Configure your director port (default: 9000)"
echo "4. For remote access: Use ngrok to expose your port"
echo ""