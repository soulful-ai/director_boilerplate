...
[![smithery badge](https://smithery.ai/badge/cli_use)](https://smithery.ai/protocol/cli_use)
[![Python Tests](https://github.com/MladenSU/cli_use/actions/workflows/python-tests.yml/badge.svg)](https://github.com/MladenSU/cli_use/actions/workflows/python-tests.yml)
...
To install CLI MCP Server for Claude Desktop automatically via [Smithery](https://smithery.ai/protocol/cli_use):

```bash
npx @smithery/cli install cli_use --client claude
```
...
```json
{
  "mcpServers": {
    "cli_use": {
      "command": "uv",
      "args": ["--directory", "<path/to/the/repo>/cli_use", "run", "cli_use"],
      "env": {
...
```bash
npx @modelcontextprotocol/inspector uv --directory {{your source code local directory}}/cli_use run cli_use
```
